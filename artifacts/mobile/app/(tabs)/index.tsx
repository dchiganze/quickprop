  ...
  return (
    ...
    >
      ...
      <View style={[styles.statsRow, { marginTop: 10 }]}>
        <StatCard label="Viewings Today" value={viewingsToday} icon="eye-outline" color={colors.accent} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {}); router.navigate({ pathname: '/(tabs)/tasks', params: { filter: 'viewing', _t: String(Date.now()) } }); }} />
        <StatCard label="Tasks Due" value={overdueTasks} icon="time-outline" color={overdueTasks > 0 ? colors.destructive : colors.mutedForeground} subtitle={overdueTasks > 0 ? 'Overdue' : 'On track'} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {}); router.navigate({ pathname: '/(tabs)/tasks', params: { filter: 'all', _t: String(Date.now()) } }); }} />
      </View>
